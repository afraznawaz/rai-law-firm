...
export default function NoorChatbot({ enabled = true }: { enabled?: boolean }) {
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      ...
      const res = await fetch('/api/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: msg, sessionId: sessionId.current, history }) })
      const data = await res.json()
      setMessages(prev => [...prev, { role: 'assistant', content: data.reply, time: formatTime() }])
    } catch {
      setMessages(prev => [...prev, { role: 'assistant', content: 'Network issue. Please contact us:\n\n💬 **WhatsApp:** 0316-4371096\n📞 **Call:** 0304-4840937', time: formatTime() }])
    } finally { setLoading(false) }
  }

  const restart = () => {
    sessionStorage.removeItem('noor_session'); sessionId.current = getSessionId()
    setMessages([{ role: 'assistant', content: WELCOME, time: formatTime() }])
  ...
  return (
    <>
      <button className={`noor-toggle ${open ? 'noor-toggle--open' : ''}`} onClick={() => setOpen(!open)} aria-label="Chat with Noor">
        {open ? <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" width="22" height="22"><path d="M18 6L6 18M6 6l12 12"/></svg> : <svg viewBox="0 0 24 24" fill="currentColor" width="24" height="24"><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-2 12H6v-2h12v2zm0-3H6V9h12v2zm0-3H6V6h12v2z"/></svg>}
      ...
      <div className={`noor-window ${open ? 'noor-window--open' : ''}`}>
        <div className="noor-header">
          <div className="noor-header__left">
            <div className="noor-header__avatar"><span>N</span><div className="noor-header__online" /></div>
            <div><div className="noor-header__name">Noor</div><div className="noor-header__sub">R&A Legal Assistant • Online</div></div>
          </div>
          <div className="noor-header__actions">
            <button className="noor-header__btn" onClick={restart} title="Restart">↺</button>
        ...
        <div className="noor-messages">
          {messages.map((msg, i) => (
            <div key={i} className={`noor-msg noor-msg--${msg.role}`}>
              {msg.role === 'assistant' && <div className="noor-msg__avatar">N</div>}
              <div className="noor-msg__bubble">
                ...
                {msg.role === 'assistant' && (msg.content.toLowerCase().includes('contact') || msg.content.toLowerCase().includes('consultation')) && i > 0 && (
          ...
          {loading && <div className="noor-msg noor-msg--assistant"><div className="noor-msg__avatar">N</div><div className="noor-msg__bubble"><div className="noor-typing"><span /><span /><span /></div></div></div>}
          {showQuick && messages.length <= 1 && !loading && (
            <div className="noor-quick">
              <div className="noor-quick__label">Quick questions:</div>
              {QUICK_QUESTIONS.map((q, i) => <button key={i} className="noor-quick__btn" onClick={() => sendMessage(q)}>{q}</button>)}
            </div>
          )}
          <div ref={messagesEndRef} />
        </div>
        <div className="noor-input-area">
          <input ref={inputRef} className="noor-input" placeholder="Type your legal question..." value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendMessage()} disabled={loading} maxLength={500} />
          <button className={`noor-send ${input.trim() ? 'noor-send--active' : ''}`} onClick={() => sendMessage()} disabled={loading || !input.trim()}>
