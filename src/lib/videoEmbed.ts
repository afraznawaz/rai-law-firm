...
export function detectVideo(url: string): VideoInfo {
  ...
  if (vimeoMatch) {
    ...
    return {
      platform: 'vimeo',
      embedUrl: `https://player.vimeo.com/video/${id}?color=c9a84c&title=0&byline=0`,
      thumbnailUrl: null,
      originalUrl: u,
      videoId: id,
    }
  ...
  if (u.includes('facebook.com') || u.includes('fb.watch')) {
    ...
    return {
      platform: 'facebook',
      embedUrl: `https://www.facebook.com/plugins/video.php?href=${encoded}&show_text=false&width=640&autoplay=false`,
      thumbnailUrl: null,
      originalUrl: u,
      videoId: null,
    }
