...
export default function Certificates({ onBack }: { onBack: () => void }) {
  ...
  const getGDriveEmbed = (url: string) => { const match = url.match(/\/d\/([a-zA-Z0-9_-]+)/); if (match) return `https://drive.google.com/file/d/${match[1]}/preview`; return url }
  const getGDriveView = (url: string) => { const match = url.match(/\/d\/([a-zA-Z0-9_-]+)/); if (match) return `https://drive.google.com/file/d/${match[1]}/view`; return url }
  ...
  return (
    <div className="cert-page">
      ...
      <div className="cert-container">
        {user && <div className="cert-admin-bar"><button className="cert-add-btn" onClick={() => setShowForm(!showForm)}>{showForm ? '✕ Cancel' : '+ Add Certificate'}</button></div>}
        {showForm && user && (
          ...
          <div className="cert-grid">
            {certs.map(cert => (
              <div key={cert.id} className="cert-card">
                <div className="cert-card__preview">{cert.file_url ? <iframe src={getGDriveEmbed(cert.file_url)} className="cert-card__iframe" title={cert.title} allowFullScreen /> : <div className="cert-card__no-preview">📄</div>}</div>
                <div className="cert-card__info">
                  ...
                  <div className="cert-card__actions">{cert.file_url && <a href={getGDriveView(cert.file_url)} target="_blank" rel="noopener noreferrer" className="cert-card__view">🔍 View Full</a>}{user && <button className="cert-card__del" onClick={() => setDeleteId(cert.id)}>🗑️</button>}</div>
                </div>
              </div>
            ))}
          </div>
