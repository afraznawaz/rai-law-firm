...
const EMPTY: Partial<Certificate> = {
  title: '', issuer: '', date: '', description: '', image_url: '', category: 'Bar Membership', published: true
}
    ...
    try {
      ...
      reader.onload = async (ev) => {
        ...
        const res = await fetch('/api/upload', {
          ...
          body: JSON.stringify({ fileName: file.name, fileType: file.type, fileData: base64, bucket: 'certificates' })
        })
        const data = await res.json()
        if (data.url) {
      ...
      {view === 'list' && (
        <>
          <div className="adm-header">
            ...
            <button className="adm-btn adm-btn--gold" onClick={() => { setForm({...EMPTY}); setUploadProgress(''); setView('new') }}>+ Add Certificate</button>
          </div>
              ...
              {items.map(item => (
                <div key={item.id} className="adm-cert-card">
                  ...
                  <div className="adm-cert-card__actions">
                    <button className="adm-btn adm-btn--sm adm-btn--outline" onClick={() => { setForm({...item}); setUploadProgress(''); setView('edit') }}>✏️ Edit</button>
            ...
            <div className="adm-editor__grid">
              <div className="adm-editor__main">
                ...
                <div className="adm-form__group">
                  ...
                  <textarea rows={3} placeholder="Brief description of this certificate..." value={form.description} onChange={e => setForm({...form, description: e.target.value})} />
                </div>

                {/* IMAGE UPLOAD */}
                <div className="adm-upload-box">
                  <h4 className="adm-upload-box__title">🖼️ Certificate Image</h4>
                  <p className="adm-upload-box__hint">Upload a photo or scan of the certificate (JPG, PNG, WebP)</p>
                  <input ref={fileRef} type="file" accept=".jpg,.jpeg,.png,.webp" onChange={handleImageUpload} className="adm-upload-box__input" />
