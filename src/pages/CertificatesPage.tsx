...
> {
  ...
  componentDidCatch(error: Error, info: { componentStack: string }) { console.error('[CertificatesPage]', error.message, info.componentStack) }
  render() {
    if (this.state.hasError) {
      ...
      return (
        <div style={{ padding: '80px 24px', textAlign: 'center', background: '#f8f5ef', minHeight: '60vh' }}>
          ...
          <button onClick={() => this.setState({ hasError: false, error: null })} style={{ padding: '10px 24px', background: '#0d3d1e', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 700 }}>Try Again</button>
        </div>
      )
    }
...
interface Cert { id: number; title: string; description: string; file_url: string; issued_by: string; issued_date: string; category: string; created_at: string }
...
function CertificatesPageInner({ onBack }: Props) {
      ...
      <div className="cert-detail">
        ...
        <div className="cert-detail__wrap">
          ...
          <div className="cert-detail__info">
            ...
            {selected.file_url && <a href={selected.file_url} target="_blank" rel="noopener noreferrer" className="cert-detail__download">{isPdf(selected.file_url) ? '📄 Open PDF' : '🖼️ View Full Certificate'}</a>}
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="cert-page">
      ...
      {loading ? (
        <div className="cert-grid" style={{ padding: '36px 32px', maxWidth: '1200px', margin: '0 auto' }}>{[1,2,3,4,5,6].map(i => <div key={i} className="ne-skeleton" />)}</div>
      ) : error ? (
        <div className="ne-empty"><div style={{ fontSize: '3rem', marginBottom: '12px' }}>⚠️</div><p style={{ fontWeight: 700, color: '#c00', marginBottom: '8px' }}>Error loading certificates</p><p style={{ fontSize: '0.85rem', color: '#666', marginBottom: '16px' }}>{error}</p><button className="ne-retry" onClick={load}>🔄 Retry</button></div>
      ) : items.length === 0 ? (
        <div className="ne-empty"><div style={{ fontSize: '3rem', marginBottom: '12px' }}>🎖️</div><p>No certificates added yet.</p><p style={{ fontSize: '0.85rem', color: '#888', marginTop: '8px' }}>Go to <strong>/admin</strong> to add certificates.</p></div>
      ) : (
...
export default function CertificatesPage({ onBack }: Props) {
  return <ErrorBoundary name="CertificatesPage"><CertificatesPageInner onBack={onBack} /></ErrorBoundary>
}
