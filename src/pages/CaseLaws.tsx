...
interface CaseLaw {
  id: number; title: string; citation: string; court: string; year: number
  summary: string; file_url: string; category: string; created_at: string
}
  ...
  return (
    <div className="cl-page">
      ...
      <div className="cert-container">
        ...
        {showForm && user && (
          <form className="cert-form" onSubmit={handleAdd}>
            ...
            <div className="cert-form__grid">
              <div className="cert-form__group cert-form__group--full"><label>Case Title *</label><input required value={form.title} onChange={e => setForm({...form, title: e.target.value})} /></div>
              <div className="cert-form__group"><label>Citation</label><input value={form.citation} onChange={e => setForm({...form, citation: e.target.value})} /></div>
              ...
              <div className="cert-form__group cert-form__group--full"><label>Google Drive Link</label><input value={form.file_url} onChange={e => setForm({...form, file_url: e.target.value})} /></div>
              <div className="cert-form__group cert-form__group--full"><label>Summary *</label><textarea required rows={4} value={form.summary} onChange={e => setForm({...form, summary: e.target.value})} /></div>
            </div>
        ...
        {loading ? <div className="cert-loading">{[1,2,3].map(i => <div key={i} className="cert-skeleton" />)}</div> : filtered.length === 0 ? <div className="cert-empty"><div className="cert-empty__icon">⚖️</div><p>No cases found.</p></div> : (
      ...
      {openCase && <div className="cl-modal-overlay" onClick={() => setOpenCase(null)}><div className="cl-modal" onClick={e => e.stopPropagation()}><button className="cl-modal__close" onClick={() => setOpenCase(null)}>✕</button><div className="cl-modal__cat">{openCase.category}</div><h2 className="cl-modal__title">{openCase.title}</h2><div className="cl-modal__meta">{openCase.citation && <span>📌 {openCase.citation}</span>}<span>🏛️ {openCase.court}</span><span>📅 {openCase.year}</span></div><div className="cl-modal__summary">{openCase.summary}</div>{openCase.file_url && <div className="cl-modal__preview"><iframe src={getGDriveEmbed(openCase.file_url)} className="cl-modal__iframe" title={openCase.title} allowFullScreen /></div>}{openCase.file_url && <a href={openCase.file_url} target="_blank" rel="noopener noreferrer" className="cert-card__view" style={{marginTop:'12px',display:'inline-block'}}>🔍 View Full Document</a>}</div></div>}
