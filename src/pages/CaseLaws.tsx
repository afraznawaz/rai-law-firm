          ...
          <form className="cert-form" onSubmit={handleAdd}>
            ...
            <div className="cert-form__grid">
              <div className="cert-form__group cert-form__group--full">
                ...
                <input required placeholder="e.g. Commissioner vs. XYZ Corporation" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} />
              </div>
              <div className="cert-form__group">
                <label>Citation</label>
                <input placeholder="e.g. 2023 PTD 123" value={form.citation} onChange={e => setForm({ ...form, citation: e.target.value })} />
              </div>
              <div className="cert-form__group">
                <label>Year</label>
              ...
              <div className="cert-form__group cert-form__group--full">
                ...
                <input placeholder="https://drive.google.com/file/d/..." value={form.file_url} onChange={e => setForm({ ...form, file_url: e.target.value })} />
                <span className="cert-form__hint">Drive → Right click → Share → Anyone with link</span>
              </div>
              <div className="cert-form__group cert-form__group--full">
                <label>Case Summary *</label>
                <textarea required rows={4} placeholder="Brief summary of the case, key legal points, and judgment..." value={form.summary} onChange={e => setForm({ ...form, summary: e.target.value })} />
              </div>
            </div>
        ...
        <div className="cl-toolbar">
          ...
          <div className="cl-filters">
            {allCategories.map(cat => (
              <button key={cat} className={`ra-blog__filter ${filter === cat ? 'active' : ''}`} onClick={() => setFilter(cat)}>{cat}</button>
            ))}
          </div>
        </div>

        {loading ? (
          <div className="cert-loading">{[1,2,3].map(i => <div key={i} className="cert-skeleton" />)}</div>
        ) : filtered.length === 0 ? (
          <div className="cert-empty">
            <div className="cert-empty__icon">⚖️</div>
            <p>{cases.length === 0 ? 'Case laws will appear here once added.' : 'No cases found.'}</p>
          </div>
        ) : (
            ...
            {filtered.map(c => (
              <div key={c.id} className="cl-card" onClick={() => setOpenCase(c)}>
                <div className="cl-card__top">
                  <span className="cl-card__cat">{c.category}</span>
                  <span className="cl-card__year">{c.year}</span>
                </div>
                ...
                <div className="cl-card__footer">
                  <button className="cl-card__read">Read More →</button>
                  {user && <button className="cert-card__del" onClick={e => { e.stopPropagation(); setDeleteId(c.id) }}>🗑️</button>}
                </div>
              </div>
            ))}
          </div>
      ...
      {openCase && (
        <div className="cl-modal-overlay" onClick={() => setOpenCase(null)}>
          <div className="cl-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="cl-modal__meta">
              {openCase.citation && <span>📌 {openCase.citation}</span>}
              <span>🏛️ {openCase.court}</span>
              <span>📅 {openCase.year}</span>
            </div>
            <div className="cl-modal__summary">{openCase.summary}</div>
            {openCase.file_url && (
              <div className="cl-modal__preview">
                <iframe src={getGDriveEmbed(openCase.file_url)} className="cl-modal__iframe" title={openCase.title} allowFullScreen />
              </div>
            )}
            {openCase.file_url && (
              <a href={openCase.file_url} target="_blank" rel="noopener noreferrer" className="cert-card__view" style={{marginTop:'12px',display:'inline-block'}}>🔍 View Full Document</a>
            )}
          </div>
        </div>
      )}
