...
export default function Admin() {
  ...
  const savePost = async (e: React.FormEvent) => {
    ...
    const token = await getToken()
    const method = blogView === 'new' ? 'POST' : 'PUT'
    ...
    if (res.ok) { setBlogMsg('✅ Saved!'); await fetchPosts(); setTimeout(() => { setBlogMsg(''); setBlogView('list') }, 1500) }
    else setBlogMsg('❌ Error saving')
    setBlogSaving(false)
  }
  const deletePost = async (id: number) => {
    ...
    setDeleteBlogId(null); await fetchPosts()
  }

  // ---- E-LIBRARY ----
  const fetchBooks = async () => {
    setBooksLoading(true)
    const res = await fetch('/api/elibrary')
    const d = await res.json()
    setBooks(Array.isArray(d) ? d : [])
    setBooksLoading(false)
  }
  const saveBook = async (e: React.FormEvent) => {
    e.preventDefault(); setLibSaving(true); setLibMsg('')
    const token = await getToken()
    const method = libView === 'new' ? 'POST' : 'PUT'
    const res = await fetch('/api/elibrary', { method, headers:{ 'Content-Type':'application/json', Authorization:`Bearer ${token}` }, body: JSON.stringify(editBook) })
    if (res.ok) { setLibMsg('✅ Saved!'); await fetchBooks(); setTimeout(() => { setLibMsg(''); setLibView('list') }, 1500) }
    else setLibMsg('❌ Error saving')
    setLibSaving(false)
  }
  const deleteBook = async (id: number) => {
    const token = await getToken()
    await fetch('/api/elibrary', { method:'DELETE', headers:{ 'Content-Type':'application/json', Authorization:`Bearer ${token}` }, body: JSON.stringify({ id }) })
    setDeleteLibId(null); await fetchBooks()
  }
  ...
  const markRead = async (msg: any) => {
    const token = await getToken()
    await fetch('/api/messages', { method:'PUT', headers:{ 'Content-Type':'application/json', Authorization:`Bearer ${token}` }, body: JSON.stringify({ id: msg.id, read: true }) })
    setOpenMsg({ ...msg, read: true })
    await fetchMessages()
  }
  const deleteMsg = async (id: number) => {
    const token = await getToken()
    await fetch('/api/messages', { method:'DELETE', headers:{ 'Content-Type':'application/json', Authorization:`Bearer ${token}` }, body: JSON.stringify({ id }) })
    setDeleteMsgId(null); setOpenMsg(null); await fetchMessages()
  }
    ...
    <div className="adm-root">
      ...
      <aside className="adm-sidebar">
        ...
        <nav className="adm-sidebar__nav">
          ...
          <button className={`adm-sidebar__link ${section==='messages'?'active':''}`} onClick={() => { setSection('messages'); setOpenMsg(null) }}>
            💬 Messages {unread > 0 && <span className="adm-badge">{unread}</span>}
          </button>
      ...
      <main className="adm-main">
        ...
        {section === 'blog' && (
          <div>
            {blogView === 'list' && (
              <>
                <div className="adm-header">
                  <div><h1 className="adm-header__title">📝 Blog Posts</h1><p className="adm-header__sub">{posts.length} total posts</p></div>
                  <button className="adm-btn adm-btn--gold" onClick={() => { setEditPost(EMPTY_POST); setBlogView('new') }}>+ New Post</button>
                </div>
                {postsLoading ? <div className="adm-loading">Loading...</div> : (
                  <div className="adm-posts">
                    {posts.map(p => (
                      <div key={p.id} className="adm-post-card">
                        <div className="adm-post-card__left">
                          <span className={`adm-post-card__status ${p.published?'published':'draft'}`}>{p.published?'🟢 Published':'🟡 Draft'}</span>
                          <h3 className="adm-post-card__title">{p.title}</h3>
                          <div className="adm-post-card__meta">
                            <span className="adm-post-card__cat">{p.category}</span>
                            <span>{new Date(p.created_at).toLocaleDateString('en-PK',{day:'numeric',month:'short',year:'numeric'})}</span>
                          </div>
                          <p className="adm-post-card__excerpt">{p.excerpt?.substring(0,100)}...</p>
                        </div>
                        <div className="adm-post-card__actions">
                          <button className="adm-btn adm-btn--sm adm-btn--outline" onClick={() => { setEditPost({...p}); setBlogView('edit') }}>✏️ Edit</button>
                          <button className="adm-btn adm-btn--sm adm-btn--danger" onClick={() => setDeleteBlogId(p.id)}>🗑️ Delete</button>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {(blogView==='new'||blogView==='edit') && (
              <>
                <div className="adm-header">
                  <div><h1 className="adm-header__title">{blogView==='new'?'New Post':'Edit Post'}</h1></div>
                  <button className="adm-btn adm-btn--outline" onClick={() => setBlogView('list')}>← Back</button>
                </div>
                <form onSubmit={savePost} className="adm-editor">
                  <div className="adm-editor__grid">
                    ...
                    <div className="adm-editor__sidebar">
                      <div className="adm-editor__panel">
                        ...
                        <div className="adm-form__group"><label>URL Slug</label><input value={editPost.slug} onChange={e => setEditPost({...editPost, slug:e.target.value})} /><div className="adm-form__hint">Auto from title</div></div>
        ...
        {section === 'elibrary' && (
          <div>
            {libView === 'list' && (
              <>
                <div className="adm-header">
                  <div><h1 className="adm-header__title">📚 E-Library</h1><p className="adm-header__sub">{books.length} documents</p></div>
                  <button className="adm-btn adm-btn--gold" onClick={() => { setEditBook(EMPTY_LIB); setLibView('new') }}>+ Add Document</button>
                </div>
                {booksLoading ? <div className="adm-loading">Loading...</div> : (
                  <div className="adm-lib-grid">
                    {books.map(b => (
                      <div key={b.id} className="adm-lib-card">
                        <div className="adm-lib-card__icon">📄</div>
                        <div className="adm-lib-card__info">
                          <div className="adm-lib-card__cat">{b.category}</div>
                          <h3 className="adm-lib-card__title">{b.title}</h3>
                          <div className="adm-lib-card__year">{b.year}</div>
                          <p className="adm-lib-card__sum">{b.summary?.substring(0,80)}...</p>
                        </div>
                        <div className="adm-lib-card__actions">
                          <button className="adm-btn adm-btn--sm adm-btn--outline" onClick={() => { setEditBook({...b}); setLibView('edit') }}>✏️</button>
                          <button className="adm-btn adm-btn--sm adm-btn--danger" onClick={() => setDeleteLibId(b.id)}>🗑️</button>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {(libView==='new'||libView==='edit') && (
              <>
                <div className="adm-header">
                  <div><h1 className="adm-header__title">{libView==='new'?'Add Document':'Edit Document'}</h1></div>
                  <button className="adm-btn adm-btn--outline" onClick={() => setLibView('list')}>← Back</button>
                </div>
                <form onSubmit={saveBook} className="adm-editor">
                  <div className="adm-editor__grid">
                    <div className="adm-editor__main">
                    ...
                    <div className="adm-editor__sidebar">
                      <div className="adm-editor__panel">
                        ...
                        <button type="submit" className="adm-btn adm-btn--gold adm-btn--full" disabled={libSaving}>{libSaving?'Saving...':libView==='new'?'📚 Add Document':'💾 Save Changes'}</button>
                      </div>
                    </div>
                  </div>
                </form>
        ...
        {section === 'messages' && (
          <div>
            <div className="adm-header">
              <div>
                <h1 className="adm-header__title">💬 Messages</h1>
                <p className="adm-header__sub">{messages.length} total · {unread} unread</p>
              </div>
              <button className="adm-btn adm-btn--outline" onClick={fetchMessages}>🔄 Refresh</button>
            </div>
            {msgsLoading ? <div className="adm-loading">Loading messages...</div> : (
              <div className="adm-msgs-layout">
                {/* Message List */}
                <div className="adm-msgs-list">
                  {messages.length === 0 && <div className="adm-msgs-empty">📭 No messages yet</div>}
                  {messages.map(m => (
                    <div key={m.id}
                      className={`adm-msg-item ${!m.read?'unread':''} ${openMsg?.id===m.id?'active':''}`}
                      onClick={() => { setOpenMsg(m); if (!m.read) markRead(m) }}>
                      <div className="adm-msg-item__top">
                        <span className="adm-msg-item__name">{m.name}</span>
                        {!m.read && <span className="adm-msg-item__dot" />}
                      </div>
                      <div className="adm-msg-item__sub">{m.subject || 'General Inquiry'}</div>
                      <div className="adm-msg-item__preview">{m.message?.substring(0,60)}...</div>
                      <div className="adm-msg-item__date">{new Date(m.created_at).toLocaleDateString('en-PK',{day:'numeric',month:'short',year:'numeric'})}</div>
                    </div>
                  ))}
                </div>
                ...
                <div className="adm-msg-detail">
                  {!openMsg ? (
                    <div className="adm-msg-detail__empty">
                      <div className="adm-msg-detail__empty-icon">💬</div>
                    ...
                    <>
                      <div className="adm-msg-detail__header">
                        <div>
                          <h2 className="adm-msg-detail__subject">{openMsg.subject || 'General Inquiry'}</h2>
                          <div className="adm-msg-detail__from">From: <strong>{openMsg.name}</strong></div>
                        </div>
                        <button className="adm-btn adm-btn--sm adm-btn--danger" onClick={() => setDeleteMsgId(openMsg.id)}>🗑️ Delete</button>
                      </div>
                      <div className="adm-msg-detail__meta">
                        {openMsg.phone && <span>📞 {openMsg.phone}</span>}
                        {openMsg.email && <span>✉️ {openMsg.email}</span>}
                        <span>🕐 {new Date(openMsg.created_at).toLocaleString('en-PK')}</span>
                      </div>
                      <div className="adm-msg-detail__body">{openMsg.message}</div>
                      <div className="adm-msg-detail__actions">
                        {openMsg.phone && <a href={`tel:${openMsg.phone}`} className="adm-btn adm-btn--gold">📞 Call Back</a>}
                        {openMsg.phone && <a href={`https://wa.me/92${openMsg.phone.replace(/^0/,'')}`} target="_blank" rel="noopener noreferrer" className="adm-btn adm-btn--outline">💬 WhatsApp</a>}
                        {openMsg.email && <a href={`mailto:${openMsg.email}`} className="adm-btn adm-btn--outline">✉️ Email Reply</a>}
                      </div>
                    </>
                  )}
                </div>
      ...
      {/* DELETE MODALS */}
      {deleteBlogId && (
        <div className="adm-modal-overlay" onClick={() => setDeleteBlogId(null)}>
          <div className="adm-modal" onClick={e => e.stopPropagation()}>
            <h3>Delete Post?</h3><p>This cannot be undone.</p>
            <div className="adm-modal__actions">
              <button className="adm-btn adm-btn--outline" onClick={() => setDeleteBlogId(null)}>Cancel</button>
              <button className="adm-btn adm-btn--danger" onClick={() => deletePost(deleteBlogId)}>Delete</button>
            </div>
          </div>
        </div>
      )}
      {deleteLibId && (
        <div className="adm-modal-overlay" onClick={() => setDeleteLibId(null)}>
          <div className="adm-modal" onClick={e => e.stopPropagation()}>
            <h3>Delete Document?</h3><p>This cannot be undone.</p>
            <div className="adm-modal__actions">
              <button className="adm-btn adm-btn--outline" onClick={() => setDeleteLibId(null)}>Cancel</button>
              <button className="adm-btn adm-btn--danger" onClick={() => deleteBook(deleteLibId)}>Delete</button>
            </div>
          </div>
        </div>
      )}
      {deleteMsgId && (
        <div className="adm-modal-overlay" onClick={() => setDeleteMsgId(null)}>
          <div className="adm-modal" onClick={e => e.stopPropagation()}>
            <h3>Delete Message?</h3><p>This cannot be undone.</p>
            <div className="adm-modal__actions">
              <button className="adm-btn adm-btn--outline" onClick={() => setDeleteMsgId(null)}>Cancel</button>
              <button className="adm-btn adm-btn--danger" onClick={() => deleteMsg(deleteMsgId)}>Delete</button>
            </div>
          </div>
        </div>
      )}
